...
Le choix Rapide / Auto / Approfondi est mémorisé sur l’appareil et enregistré avec chaque question. Rapide et Auto conservent Flash pour l’analyse, la recherche web, la synthèse, la réparation et la relecture indépendante. Approfondi utilise Pro pour l’analyse et la relecture à la demande explicite du brasseur ; ses recherches restent confiées à Flash.
...
Une question simple utilise une analyse et une relecture indépendante, avec les calculs déterministes de l’application. Pour un achat, trois chercheurs Flash travaillent en parallèle : produit exact, autre boutique et variantes, puis conditionnement et disponibilité annoncée. Ils ne reçoivent que la recherche courte, sans les factures ni le registre comptable. Le serveur lit les fiches réelles, puis la synthèse sélectionne les offres pertinentes. Deux relecteurs Flash distincts contrôlent en parallèle la justesse technique et l’utilité concrète pour le brasseur ; cette double relecture s’applique aussi aux recherches documentaires, aux finances, aux propositions de champs et aux gestes sensibles. Les deux avis doivent approuver le résultat.

Les corrections réutilisent les preuves existantes ; une nouvelle recherche ciblée reste possible si une information décisive manque, dans la limite totale de six tentatives avec Google Search par question, replis compris. Un achat peut donc mobiliser une seconde vague de trois chercheurs sans multiplier les recherches en boucle. Les appels indépendants se déroulent en parallèle et chaque étape dépendante attend leurs résultats, conformément au [protocole d’appels de fonctions Gemini](https://ai.google.dev/gemini-api/docs/generate-content/function-calling). Pour Flash, chaque recherche ou analyse est limitée à 45 secondes et chaque relecture à 30 secondes, dans la durée globale du travail. Aucun agent ne modifie les limites ni n’effectue un achat.

## Plafonds et arrêt

`brewerBudget.ts` réserve une marge de tokens avant **chaque** appel payé, dans une transaction Firestore commune à toutes les conversations. Les plafonds par défaut viennent de `DEFAULT_BREWER_LIMITS` ; chaque question dispose de 24 appels au maximum, reprises automatiques comprises. Le jour suit Europe/Zurich. Le nombre d’appels et de recherches web reste plafonné même si le fournisseur ne renvoie pas ses métadonnées d’usage. Une panne de contrôle bloque les nouveaux appels.
...
`getBrewerAiBudget` et `setBrewerAiBudget` sont réservés au brasseur connecté. Le menu replié « Limites IA » montre l’usage, suspend/réactive le compagnon et pose ses cinq plafonds en champs directement modifiables — trois pour la journée, deux par question. Les bornes acceptées viennent de `BREWER_LIMIT_BOUNDS` (`functions/src/brewerLimits.ts`), partagées par l’écran et le serveur ; l’écran y ramène chaque valeur avant l’envoi, puisque `NumberInput` n’applique les siennes qu’à la sortie du champ. Un abonnement serveur à `brewerAiControls/current` interrompt les requêtes en cours lors d’une suspension ; les appels suivants sont aussi bloqués par la transaction. Enregistrer des plafonds ne réactive jamais une suspension concurrente. Une limite atteinte reste une erreur explicite et ne déclenche pas de reprise automatique. Une relance manuelle est possible après réactivation, sans effacer le compteur du jour.

Les limites quotidiennes du compagnon s’ajoutent au [budget mensuel commun](gemini-budget.md), choisi ici à 20 CHF pour tous les appels Gemini de l’application, recherche comprise. Ce suivi ne plafonne pas toute la facturation Google Cloud. Un appel transmis avant l’arrêt peut continuer d’être facturé par le fournisseur.
