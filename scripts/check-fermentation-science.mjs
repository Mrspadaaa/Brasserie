...
try{
 for(const width of [390,320,1280]){
  ...
  await button(page,'📜 Recettes');await button(page,'Créer');await page.locator('#wz-title').fill('Fermentation recherche '+width);
  ...
  await page.goto(base+'/research/fermentation-2026.html',{waitUntil:'networkidle0'});await overflow(page);
  assert.equal(await page.$$eval('h1',els=>els.length),1);assert(await page.$$eval('a[href^="https://"]',els=>els.length)>=20);
  await page.screenshot({path:resolve(out,'rapport-ouverture-'+width+'.png')});
  await page.evaluate(()=>[...document.querySelectorAll('h2')].find(h=>h.textContent.includes('Phénols')).scrollIntoView({block:'start'}));
  await page.screenshot({path:resolve(out,'rapport-phenols-'+width+'.png')});
  await page.evaluate(()=>scrollTo(0,document.documentElement.scrollHeight));await page.screenshot({path:resolve(out,'rapport-fin-'+width+'.png')});
  assert.deepEqual(errors,[]);reports.push({width,autocomplete:true,lagerRange:true,localModel:true,outOfDomain:true,noWrites:true,reportLinks:true,errors});
