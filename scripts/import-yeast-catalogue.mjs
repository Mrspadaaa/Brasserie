...
const incoming=JSON.parse(await readFile('src/data/yeastCatalogueBootstrap.json','utf8'));
...
if(apply){
  for(let start=0;start<plan.writes.length;start+=200){const batch=plan.writes.slice(start,start+200);
    await request(base+':commit',{writes:batch.map(w=>({update:{name:parent+'/hopKnowledge/'+w.id,fields:encode(w.data).mapValue.fields},...(w.create?{}:{updateMask:{fieldPaths:['catalogue']}}),currentDocument:w.create?{exists:false}:{updateTime:w.updateTime}}))});
