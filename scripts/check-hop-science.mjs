...
const compiled = await build({
  absWorkingDir: root, entryPoints: [resolve(root, 'tests/scientific/hopBenchmark.ts')],
...
const { runHopScientificBenchmark } = await import('data:text/javascript;base64,' + Buffer.from(compiled.outputFiles[0].text).toString('base64'));
