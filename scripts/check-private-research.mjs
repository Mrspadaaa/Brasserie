...
const noloMarker = readFileSync(join(root,'functions/reports/nolo-2026.md'),'utf8').trim().split(/\r?\n\r?\n/)[1];
const scenarioPath = join(root,'functions/reports/nolo-scenarios-2026.md');
const scenarioMarker = readFileSync(existsSync(scenarioPath)?scenarioPath:join(root,'functions/reports/nolo-scenarios.example.md'),'utf8').trim().split(/\r?\n\r?\n/)[1];
const failures = [];
...
for (const base of ['public', 'dist']) {
  for (const file of walk(join(root, base))) {
    ...
    else if (/\.(html|js|json|md|map)$/.test(file) && [marker,noloMarker,scenarioMarker].some(m=>readFileSync(file, 'utf8').includes(m))) failures.push(relative(root, file));
  }
}
