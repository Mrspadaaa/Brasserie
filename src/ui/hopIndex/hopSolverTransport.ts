...
import type { HopSearchMessage } from './hopSolverMessages';
...
export function startHopSolverSearch(input:HopSolverSearchOptions,publish:(update:HopSearchUpdate)=>void,fail:(error:Error)=>void,
  ...
  let lastResults:HopSearchUpdate['results']|undefined;
  ...
  try{
    ...
    worker.onmessage=event=>{
      const m=event.data as HopSearchMessage;
      if(m?.kind==='update'){lastResults=m.update.results;update(m.update);}
      else if(m?.kind==='progress'&&lastResults)update({...m.update,results:lastResults});
      else if(m?.kind==='error')error(new Error(m.message));
      else error(new Error('Réponse de recherche invalide.'));
    };
