...
export function HopExplorationChart({ prediction, axes, target, baseline, highlighted = [], variety, models = [], showAll = false, title = 'Profil aromatique estimé' }: {
  ...
  return <figure className="space-y-2" aria-label="Graphe de la prédiction expérimentale">
    ...
    {!showAll && documentaryEvidence.length > 0 && <details className="text-sm text-cave-200"><summary className="cursor-pointer min-h-touch text-water">Repères documentés de {variety!.name}</summary><div className="pb-3 space-y-2"><p>{documentaryAxes.map(a => a.name).join(', ')}.</p><p className="text-xs text-cave-400">Descriptions de la variété ; l’intensité dans cette bière est évaluée séparément.</p>{documentaryEvidence.map((d, i) => <div key={i}><p>{d.text}</p><HopSourceLink source={d.source} /></div>)}</div></details>}
