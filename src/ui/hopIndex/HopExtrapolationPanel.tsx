...
export function HopExtrapolationPanel({ recipe, onChange, onBusyChange, target = {}, readOnly = false, initiallyExpanded = false }: {
  ...
  return <section aria-label="Simulateur aromatique expérimental" className="space-y-5 rounded-panel border border-hop/30 bg-cave-950/40 p-3 sm:p-4">
    <header className="space-y-2"><div className="flex gap-2 items-center"><FlaskConical size={18} className="text-hop" /><h3 className="font-sans text-base font-semibold text-cave-50">Tester une combinaison libre</h3></div><p className="text-xs text-ebc-straw">Simulation expérimentale · consulter les plages et leurs sources.</p></header>
