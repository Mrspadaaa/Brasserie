...
export function HopKnowledgePanel() {
  ...
  return <section className="space-y-4" aria-label="Connaissances du houblon">
    ...
    <details className="border border-cave-700 rounded-control p-3 space-y-3"><summary className="cursor-pointer text-cave-50">Étude disponible · Cascade × Wyeast 1728</summary>
    ...
    <details className="border border-cave-700 rounded-control p-3 space-y-3"><summary className="cursor-pointer text-cave-50">Catalogue fabricant · Hopsteiner</summary>
      ...
      {draft && <div className="space-y-4 pb-4">
        ...
        {json !== null ? <HopField label="Document de connaissance" hint="Édition avancée des paramètres et de leur provenance. Les modèles et axes modifiés exigent une nouvelle version."><textarea className={`${inputClass} font-mono text-sm min-h-96 py-3`} value={json} onChange={e => setJson(e.target.value)} /></HopField> : <>
          ...
          {draft.kind === 'model' && <>
            ...
            {draft.outputs.map((o, i) => <fieldset key={i} className="border border-cave-700 p-3 space-y-3"><legend className="text-cave-50">{axes.find(a => `axis:${a.id}` === o.target)?.name || o.target}</legend>
            ...
            <label className="flex items-center gap-2 min-h-touch text-cave-50"><input type="checkbox" checked={draft.enabled} onChange={e => setDraft({ ...draft, enabled: e.target.checked })} />Activer ce modèle pour son domaine documenté</label>
          </>}
