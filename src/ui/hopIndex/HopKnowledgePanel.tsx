...
export function HopKnowledgePanel() {
  ...
  return <section className="space-y-4" aria-label="Connaissances du houblon">
    ...
    <details className="border border-ebc-straw/30 rounded-control p-3 space-y-3"><summary className="cursor-pointer min-h-touch text-ebc-straw">Levures et conduites de fermentation · Weissbier</summary>
      <p className="text-sm text-cave-300">Quatre souches, profils banane/girofle, fenêtres fabricant et paliers proposés. Les températures et durées se révisent ici avec leurs sources ; aucune concentration d’ester n’est inventée.</p>
      <Button disabled={installing} onClick={async () => {
        setInstalling(true); setError('');
        try {
          const ids = new Set(fermentationPack.filter(k => k.kind === 'fermentation').map(k => k.yeastId));
          const yeasts = guideYeasts([]).filter(y => ids.has(y.id)).map(({ aliases: _, ...y }) => y);
          await ensureGuideReferences({ knowledge: [...fermentationPack as HopKnowledge[], ...yeasts] });
