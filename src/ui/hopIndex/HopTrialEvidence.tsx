...
export function HopTrialResult({ trial }: { trial: HopTrial }) {
  return <section aria-label="Résultat documenté de l’essai" className="space-y-3 border-l-2 border-hop pl-4 py-1">
    ...
    <details className="space-y-1 text-xs text-cave-300"><summary className="cursor-pointer min-h-touch flex items-center">Limites de l’essai</summary>{trial.limitations.map((limitation, i) => <p key={i}>{limitation}</p>)}</details>
...
export function HopTrialComparison({ recipe, trial }: { recipe: TrialRecipe; trial: HopTrial }) {
  ...
  return <section aria-label="Écarts au programme documenté" className="space-y-3">
    <div><h4 className="font-semibold text-cave-100">Ta recette actuelle face à l’essai</h4><p className="text-sm text-cave-400">Les conditions concordantes rendent l’essai utile comme repère ; les différences limitent sa transposition.</p></div>
