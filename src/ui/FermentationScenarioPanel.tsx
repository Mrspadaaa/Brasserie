...
export function FermentationScenarioPanel({ recipe, yeasts, guides, science, goal, onChange, programOnly = false }: {
  ...
  return <section aria-label="Résultat de ma fermentation" className="space-y-3" data-engine={result.version}>
    ...
    {!programOnly && <><dl className="grid grid-cols-2 gap-3 text-sm">
      ...
      <div data-value="final-gravity"><dt className="text-cave-400">DF estimée</dt><dd className="text-cave-50">{result.fg.range ? fermentationRangeLabel(result.fg.range, 'SG', 3) : 'Non quantifiable'}</dd><dd className="text-sm text-cave-400">Confiance faible</dd></div>
    </dl>
    ...
    {onChange && <details><summary className="cursor-pointer min-h-touch flex items-center text-water">{programOnly ? 'Modifier les températures et durées de la recette' : 'Tester mes températures et durées'}</summary><div className="space-y-3 py-2">
      <p className="text-sm text-cave-400">{programOnly ? 'Ces consignes modifient le brouillon. Enregistre la recette pour les conserver.' : 'Modifie les consignes pour vérifier leur domaine. Aucun multiplicateur universel d’arôme n’est appliqué.'}</p>
    ...
    {!programOnly && <details><summary className="cursor-pointer min-h-touch flex items-center text-water">Arômes, chimie et portée de l’estimation</summary><div className="space-y-3 py-2">
      ...
      {recipe.yeast.attenuationPct != null && <p className="text-sm text-cave-400">Atténuation saisie : {recipe.yeast.attenuationPct} % · {recipe.yeast.attenuationBasis === 'measured' ? 'retour mesuré' : recipe.yeast.attenuationBasis === 'declared' ? 'donnée annoncée' : 'hypothèse de recette'}. La base retenue et ses limites sont indiquées ci-dessus.</p>}
      ...
      {result.temperature?.source && <HopSourceLink source={result.temperature.source}/>}
