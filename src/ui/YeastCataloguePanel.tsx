...
function FactBand({ fact }: { fact: YeastCatalogueFact }) {
  ...
  return <figure className="space-y-1"><figcaption className="text-xs text-cave-300">{YEAST_FACT_LABELS[fact.key]} · {display(fact)}</figcaption>
...
export function YeastCatalogueDetails({ yeast }: { yeast: HopYeast }) {
  ...
  return <div className="space-y-4">
    <p className="text-xs text-cave-400">Données déclarées par la source. Les plages dépendent du moût et du procédé ; elles ne sont pas des intervalles de confiance statistiques. Les contradictions restent signalées dans les caractéristiques concernées.</p>
    {bands.length>0&&<div className="grid sm:grid-cols-2 gap-4">{bands.map((f,i)=><FactBand key={i} fact={f}/>)}</div>}
    <dl className="divide-y divide-cave-700">{facts.map((f,i)=><div key={i} className="py-2 space-y-1"><dt className="text-sm font-semibold text-cave-200">{YEAST_FACT_LABELS[f.key]} <span className="font-normal text-cave-400">· {f.label}</span></dt><dd className="text-sm text-cave-100 break-words">{display(f)}</dd>{f.context&&<dd className="text-xs text-cave-400">{f.context}</dd>}<dd><HopSourceLink source={f.source}/></dd></div>)}</dl>
    {!facts.length&&<p className="text-sm text-cave-300">Référence identifiée dans le catalogue. Caractéristiques encore inconnues.</p>}
    ...
    <details className="border-t border-cave-700 pt-2"><summary className="cursor-pointer min-h-touch text-sm text-water">Sources et mises à jour</summary>
      <div className="space-y-2 text-xs text-cave-400"><p>Collecte la plus récente : {dates.at(-1)?.slice(0,10)}. Année de chaque publication indiquée dans sa source.</p><p>Présence au catalogue : {c.status==='discontinued'?'arrêt déclaré':c.status==='listed'?'répertoriée, disponibilité à vérifier':'inconnue'}.</p>
...
export function YeastCataloguePanel({ onSelect, disabled=false, selectedId, initialForm='sèche' }: { onSelect?: (yeast:HopYeast,form:YeastSpec['form'])=>void; disabled?:boolean; selectedId?:string; initialForm?:YeastSpec['form'] }) {
  ...
  return <section aria-label="Catalogue des levures" className="space-y-4">
    <div><h3 className="font-serif text-xl text-cave-50">Toutes les cultures du catalogue</h3><p className="text-sm text-cave-400 mt-1">{yeasts.length.toLocaleString('fr')} références · {manufacturers.length} fabricants et banques. Levures, mélanges et bactéries ; les références sans analyse restent visibles.</p></div>
    ...
    <p role="status" className="text-xs text-cave-400">{filtered.length.toLocaleString('fr')} référence(s) trouvée(s). Les mots aromatiques décrivent la source ; ils ne prédisent pas l’intensité dans ta bière.</p>
