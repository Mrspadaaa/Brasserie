...
export function BoundGraph({bound,target,label='Projection au conditionnement'}:{bound:NoloBound;target:number;label?:string}) {
  ...
  return <figure aria-label={label} className="space-y-1" data-nolo-min={bound.min} data-nolo-max={bound.max??'unknown'}>
    <p className="text-sm text-cave-400">{label}</p>
    <figcaption className="font-mono text-xl text-cave-50">{noloDisplayRangeLabel(bound)}</figcaption>
    {bound.max!==null&&<>
      <svg viewBox="0 0 440 80" className="w-full" role="img" aria-label="Échelle d’alcool et cible" data-nolo-scale-max={upper}>
      ...
      <p className="text-xs text-cave-400">{bound.kind==='measurement'?'Analyse fournie':bound.kind==='experimental'?'Projection conditionnelle · incertitude expérimentale non chiffrée':'Bornes physiques · pas une prédiction'} · confiance {confidence}</p>
    </>}
  </figure>;
}
