...
export function FermentationTemperatureChart({ steps, bands = [], pitchTempC }: { steps: FermentationStep[]; bands?: HopRange[]; pitchTempC?: number }) {
  ...
  return <figure aria-label="Calendrier des températures de fermentation" className="space-y-2" data-total-days={total} data-temp-min={min} data-temp-max={max}>
    ...
    <div ref={box} className="w-full min-w-0"><svg className="w-full" height="205" viewBox={`0 0 ${width} 205`} role="img" aria-label="Consignes de température en fonction des jours de fermentation">
