...
function Num({label,value,onChange,unit,max}: {label:string;value:number|null|undefined;onChange:(n:number|null)=>void;unit?:string;max?:number}) {
  const id=useId();
  return <label htmlFor={id} className="block min-w-0 text-xs text-cave-200">{label}{unit?' · '+unit:''}
    <NumberInput id={id} min={0} max={max} className={inputClass+' mt-1'} value={value??undefined} emptyValue={undefined} placeholder="Inconnu"
      onValue={n=>{if(n==null||Number.isFinite(n)&&n>=0&&(max==null||n<=max))onChange(n??null);}}/></label>;
}
export function RangeInput({label,value,onChange,unit,max}:{label:string;value:HopRange|null|undefined;onChange:(r:HopRange|null)=>void;unit:string;max?:number}) {
  ...
  return <div className="min-w-0"><p className="text-xs text-cave-200 mb-1">{label} · {unit}</p><div className="grid grid-cols-2 gap-2">
...
export function NoloPanel({recipe,onChange,allowEnable=false,measurementOnly=false,hideStrainPicker=false,onChooseYeast,showOverview=true,variantMode=false,onVariantChange}:{recipe:TrialRecipe;onChange?:(r:TrialRecipe)=>void;allowEnable?:boolean;measurementOnly?:boolean;hideStrainPicker?:boolean;onChooseYeast?:()=>void;showOverview?:boolean;variantMode?:boolean;onVariantChange?:(open:boolean)=>void}) {
  ...
  const variantStart=useRef<HTMLElement>(null),variantTrigger=useRef<HTMLSpanElement>(null),wasVariant=useRef(false);
  const variantOpen=!!variant;
  useEffect(()=>{
    onVariantChange?.(variantOpen);
    if(variantOpen){variantStart.current?.querySelector('button')?.focus();variantStart.current?.scrollIntoView({block:'start'});}
    else if(wasVariant.current)variantTrigger.current?.querySelector('button')?.focus();
    wasVariant.current=variantOpen;
  },[variantOpen,onVariantChange]);
  ...
  if(workshop&&onChange)return <section className="space-y-3"><Button onClick={()=>setWorkshop(false)}>Revenir au bilan NOLO</Button><NoloFermentationWorkshop recipe={recipe} onChange={onChange} variant={variantMode}/></section>;
  if(variant)return <section ref={variantStart} className="space-y-3"><Button onClick={()=>setVariant(undefined)}>Fermer la variante NOLO</Button><p className="text-xs text-cave-400">Variante locale · aucune écriture</p><NoloPanel recipe={variant} onChange={setVariant} variantMode/></section>;
  const preparation=editable&&!measurementOnly&&c?.enabled&&science&&<>
    <NoloRecipeSimulator recipe={recipe} onChange={onChange!} science={c.scienceSnapshot??science} saved={saved} variant={variantMode}/>
    <details><summary className="cursor-pointer">Outils complémentaires · ajouts, dilution et essais</summary>
      <NoloBrewTools recipe={recipe} onChange={onChange!} science={c.scienceSnapshot??science} result={result} saved={saved}/>
    </details>
    {!hideStrainPicker&&<details><summary className="cursor-pointer">Catalogue et profil aromatique des levures</summary>
      <Button onClick={()=>onChooseYeast?onChooseYeast():setWorkshop(true)}>Ouvrir l’atelier de fermentation</Button>
    </details>}
  </>;
  return <section aria-label="Objectif NOLO" className={`nolo-panel min-w-0 ${showOverview?'rounded-panel border border-cave-700 bg-cave-900':'nolo-panel-embedded'}`}>
    ...
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      {c&&!editable&&<span ref={variantTrigger}><Button onClick={()=>setVariant(structuredClone(recipe))}>Simuler une variante NOLO</Button></span>}
    </div>
    {c?.enabled&&<>
      {!editable&&showOverview&&<NoloRecipeOverview recipe={recipe} saved={saved}/>}
      {measurementOnly&&<div className="nolo-summary"><span>{noloProcessLabels[c.process]}</span><span>Cible ≤ {decimal(c.targetAbvPct)} % vol.</span></div>}
      ...
      {c.process==='secondRunnings'&&<div className="space-y-3" aria-label="Moût de seconde extraction">
        ...
        {editable&&!measurementOnly?<div className="grid grid-cols-2 gap-3">
          <Num label="Volume récupéré" unit="L" value={c.secondRunnings?.recoveredL} onChange={n=>update({secondRunnings:{...emptyRunnings,...c.secondRunnings,recoveredL:n}})}/>
          <Num label="Densité récupérée" unit="SG" value={c.secondRunnings?.sg} onChange={n=>update({secondRunnings:{...emptyRunnings,...c.secondRunnings,sg:n}})}/>
      ...
      {!result?<p role="alert" className="text-sm text-ebc-straw">{outcome.error||'Référence NOLO absente ou désactivée. Complète les références pour calculer ce scénario.'}</p>:<>
        ...
        {editable&&!measurementOnly&&c.process==='dealcoholized'&&c.operations.filter(o=>o.kind==='removal').map(o=>o.kind==='removal'&&<details key={o.id} open={!o.ethanolRemovedPct||!o.finalVolumeL}><summary className="min-h-touch cursor-pointer text-sm text-water">Hypothèses de désalcoolisation</summary><div className="space-y-3">
          ...
          {result.requiredRemovalPct&&<p className="text-xs text-cave-200">Retrait nécessaire avant ajouts : {decimal(result.requiredRemovalPct.min)}–{decimal(result.requiredRemovalPct.max)} %. Les apports ultérieurs doivent encore entrer dans le bilan.</p>}
        </div></details>)}
        ...
        <details><summary className="min-h-touch cursor-pointer text-sm text-water">Étapes, hypothèses et vérification</summary><div className="space-y-2 text-xs text-cave-200">
      ...
      <details><summary className="cursor-pointer min-h-touch flex items-center text-water">Moût, ajouts et mesures</summary><div className="space-y-4 pt-2">
        {editable?<><RangeInput label="Extrait du moût" unit="°P" value={c.wort.ogPlato} max={100} onChange={ogPlato=>update({wort:{...c.wort,ogPlato}})}/>
          <details><summary className="cursor-pointer min-h-touch text-sm text-cave-200">Analyse des sucres du moût</summary><div className="space-y-3">{NOLO_SUGARS.map(s=><RangeInput key={s} label={sugarLabel[s]} unit="g/L" value={c.wort.sugarsGL[s]} onChange={r=>update({wort:{...c.wort,sugarsGL:{...c.wort.sugarsGL,[s]:r}}})}/>)}</div><label className="flex gap-2 text-xs text-cave-200 py-3"><input type="checkbox" checked={c.wort.sugarsComplete} onChange={e=>update({wort:{...c.wort,sugarsComplete:e.target.checked}})}/>Profil complet : les sucres omis sont confirmés absents ; les champs effacés restent inconnus.</label></details>
          <div className="space-y-3">{c.operations.map((o,i)=>o.id==='batch-priming'?<p key={o.id} className="text-xs text-cave-200">Resucrage du brassin : {o.kind==='sugar'&&o.unclassifiedSugarG?decimal(o.unclassifiedSugarG.max)+' g':'quantité inconnue'}. À modifier dans le conditionnement du brassin.</p>:<details key={o.id}><summary className="cursor-pointer min-h-touch text-sm text-cave-200">{i+1}. {o.name||o.kind}</summary><div className="space-y-3 border-l border-cave-700 pl-3">
            <label className="block text-xs text-cave-200">Nom de l’opération<Input className={inputClass} value={o.name} onChange={e=>op(i,{...o,name:e.target.value})}/></label>
            {'volumeL'in o&&<Num label="Volume ajouté" unit="L" value={o.volumeL} onChange={n=>op(i,{...o,volumeL:n})}/>}
            {o.kind==='sugar'&&recipe.fermentables.some(f=>f.use==='fermentation')&&<label className="block text-xs">Ajout de recette déjà compté<select className={inputClass} value={o.recipeAddition?.index??''} onChange={e=>op(i,{...o,recipeAddition:e.target.value===''?undefined:{index:Number(e.target.value),basis:JSON.stringify(recipe.fermentables[Number(e.target.value)])}})}><option value="">Ajout indépendant</option>{recipe.fermentables.map((f,index)=>f.use==='fermentation'&&<option key={index} value={index}>{f.name} · {Units.format(f.weightKg,'kg')}</option>)}</select><span className="text-cave-400">Les grammes de sucres ci-dessous représentent cet ingrédient entier ; ne pas saisir une seconde opération pour le même apport.</span></label>}
            {o.kind==='sugar'&&<>{NOLO_SUGARS.map(s=><RangeInput key={s} label={sugarLabel[s]+' ajouté'} unit="g au total" value={o.sugarsG[s]} onChange={r=>op(i,{...o,sugarsG:{...o.sugarsG,[s]:r}})}/>)}<label className="flex gap-2 text-xs text-cave-200"><input type="checkbox" checked={o.complete} onChange={e=>op(i,{...o,complete:e.target.checked})}/>Composition complète de l’ajout</label></>}
      ...
      {editable&&<details><summary className="cursor-pointer min-h-touch text-water">Essais de restitution sur fractions</summary><div className="space-y-3"><p className="text-xs text-cave-400">Fractions du même brassin, sans modifier la recette entière. Garder un témoin et une hefeweisse alcoolisée comme comparaison. Aucun dosage prérempli.</p>
        {(c.trials??[]).map((t,i)=><details key={t.id}><summary className="min-h-touch cursor-pointer text-sm text-cave-200">{t.name}</summary><div className="space-y-2">
      ...
      {editable&&c.process==='secondRunnings'&&<details><summary className="cursor-pointer min-h-touch text-water">Tracer la seconde extraction</summary><div className="space-y-3">
        ...
        {([['waterAddedL','Eau ajoutée','L'],['alkalinityPpm','Alcalinité de l’eau','ppm CaCO₃'],['temperatureC','Température','°C'],['minutes','Contact','min'],['ph','pH récupéré','']] as const).map(([k,l,u])=><Num key={k} label={l} unit={u} value={c.secondRunnings?.[k]} onChange={n=>update({secondRunnings:{...emptyRunnings,...c.secondRunnings,[k]:n}})}/>)}
      ...
      <details><summary className="cursor-pointer min-h-touch text-water">Vigilances et conservation{result?.alerts.length?' · '+result.alerts.length:''}</summary><div className="space-y-2 text-sm text-cave-200">
      ...
      <details><summary className="cursor-pointer min-h-touch text-water">Comparer les procédés et consulter les sources</summary><div className="space-y-3 text-sm">
        ...
        {science?.processes.map(p=><details key={p.id}><summary className="min-h-touch cursor-pointer text-cave-50">{p.name}</summary><dl className="space-y-2 text-xs text-cave-200">{[['Arômes',p.aroma],['Travail',p.work],['Eau / énergie',p.waterEnergy],['Équipement',p.equipment],['Analyses',p.analyses],['Preuve',p.evidence],['Limites',p.limitation]].map(([k,v])=><div key={k}><dt className="font-semibold">{k}</dt><dd>{v}</dd></div>)}</dl><HopSourceLink source={p.source}/></details>)}
