...
export function InvoiceScanReview({ result, cached, expanded = false, compact = false }: { result: ScannedInvoiceResult; cached?: boolean; expanded?: boolean; compact?: boolean }) {
  ...
  if (compact) return <section aria-label="Vérification du justificatif" className={`rounded-panel border ${clear?'border-hop/30 bg-hop/5':'border-amber-400/30 bg-amber-400/5'}`}>
    <details open={expanded}>
      <summary aria-label="Détails de la lecture" className={`min-h-14 flex items-center gap-2 p-3 cursor-pointer ${clear?'text-hop':'text-amber-200'}`}>
        ...
        <div className="flex-1 min-w-0"><p className="font-medium text-sm">{documentTitles[result.documentType??'other']} · {result.items.length} article{result.items.length>1?'s':''}</p><p className="text-xs text-cave-300 mt-1"><span className="capitalize">{result.vendor}</span>{result.vendor?' · ':''}<span>{presentation.title}</span></p></div>
        <ChevronDown size={14} className="shrink-0"/>
      </summary>
      <div className="px-3 pb-3 space-y-2">
        <p className="text-xs text-cave-300 leading-relaxed">{presentation.detail}</p>
  ...
  return <section aria-label="Vérification du justificatif" className={`rounded-panel border p-3 space-y-2 ${clear ? 'border-hop/30 bg-hop/5' : 'border-amber-400/30 bg-amber-400/5'}`}>
    <div role="status" className={`flex items-start gap-2 ${clear ? 'text-hop' : 'text-amber-200'}`}>
      ...
      <div className="min-w-0"><p className="font-medium text-sm">{compact?`${documentTitles[result.documentType??'other']} · ${result.items.length} article${result.items.length>1?'s':''}`:presentation.title}</p>{compact?<p className="text-xs text-cave-300 mt-1">{presentation.title}</p>:<p className="text-xs text-cave-300 leading-relaxed mt-1">{presentation.detail}</p>}</div>
    </div>
