    ...
    for (const [name, rows] of entries) {
      ...
      for (const row of rows ?? []) {
        ...
        if (previous && !unchanged && name === 'hopKnowledge') {
          ...
          if (['model', 'axis', 'extrapolation', 'solver'].includes(previous.kind) && previous.version === row.data.version) throw Error('La connaissance importée doit porter une nouvelle version.');
        }
