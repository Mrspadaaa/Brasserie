...
export function captureHopPrediction(triplet: HopTriplet, target: Record<string, HopRange>, data: HopEngineData,
  ...
  const knowledge = usableHopKnowledge(data.knowledge).valid.filter(k => k.kind === 'note' ? false : (k.kind === 'model' || k.kind === 'extrapolation')
    ? prediction.modelRefs.some(ref => ref.id === k.id) : k.kind === 'yeast' ? k.id === triplet.yeastId : true);
  return structuredClone({ ...identity, engineVersion: prediction.extrapolatedAxes?.length ? 'hop-experimental-v3' : 'hop-envelope-v2', target, prediction,
